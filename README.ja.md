...
### `wide` — master を上に
`tall` を 90° 回転: master が上の行、 残りが下のカラムになる。直接
選択 (`--layout=wide`) も、 `tall` から `--toggle-orientation` で
反転も可。
...
`tall` / `wide` / `centered` は実行時に調整できる (per-WS)。 focus 中の
...
facet window --inc-master|--dec-master       # master 窓数 ±1 (tall / wide / centered)

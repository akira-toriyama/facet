...
dracula / github-dark / catppuccin-mocha … 全35テーマ + `random`、ライブ切替) を共有。
...
カラム数・ラベル位置等の変更はファイル内のコメントを参照。 配布
テンプレは **設定の自動永続化を既定で ON**（`[config]`・[設定](#設定)参照）＝
セッション編集を再起動後も保持する。 facet に config.toml を
一切書かせたくない場合は `[config]` ブロックをコメントアウト。
...
- `[[desktop.N.section]]` ブロック — mac desktop ごとの section モデル
  ...
  **`unassigned = true`（"lost & found" 受け皿）は RETIRED なキー**。 config に
  残っているなら（旧テンプレは *Recommended!!* と書いていた）**その section
  ブロックを削除する。 失うものは何も無い。** 管理下の窓は必ずちょうど 1 つの
  workspace に属し、 facet には受け皿に窓を入れる手段が存在しなかった — 永遠に
  空のままの section を描画していただけ。 残した行は黙って無視されない: その
  section は**まるごと DROP され、 loud に知らされる**（`facet config --validate`
  がそう言って exit 1）。 普通の workspace セルには昇格させない — それは黙って
  workspace を 1 つ増やし、 レイアウトを変えてしまうから。
  （section の `type` ももう無い — 保存フィルタは
...
- `[desktop.N]` テーブル — **typed desktop**: `type = "workspace"` か
  ...
  任意の `label`。 `[[desktop.N.section]]` ブロックも sub-workspace も持たず、
  tree にのみ描画される。
...
- `[config]` table — **設定の自動永続化 (opt-in)**。 `export-path` を
  ...
  UI なし・全自動。 コード既定は無効だが、**配布テンプレは ON**
  ＝オプトアウトは `[config]` をコメントアウト。
...
# isolate desktop 専用 — workspace section は reject。空 PREDICATE は config の
...
# Server 制御
facet --theme NAME                # 全35テーマ + random (terminal, chomp, …; config.toml 参照)

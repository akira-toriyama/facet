...
> **進捗（2026-06-25）**: ✅ **C 完了**（`facet section --focus N|label`・#337）＋ ✅ **空 section 移動時の focused-highlight stale 修正**（#337 同梱）＋ ✅ **A0 完了**（lens identity を label→安定 id へ decouple・`feat/a0-lens-identity-id`・本セッション）。次は **A**。順序 = **~~A0~~ → A → B+D → E → F**（破壊的・relay 多段）。

- ✅ **A0 lens identity を安定 id へ decouple** 完了（挙動不変リファクタ・`feat/a0-lens-identity-id`）。lens の同定キーを label 文字列 → **安定 id `"section:<declOrder>:<label>"`**（= `ProjectedSection.id`・declOrder = config FULL 配列の index）へ移行。**スコープ = minimal**（識別は Core/Adapter/Controller のみ id 化・**view モジュールは label 据え置き**＝pick `.lens(label:)`・highlight `sec.label==activeLens` のまま、`lastActiveLens=currentActiveSection.lensLabel` で id から label を parse して供給。view の id 化は A で実施）。
  - **実装**: `ActiveSection.lens` の payload を label→id へ（case 据え置き）・`lensID`(raw)/`lensLabel`(id→label parse) 追加。`ApplyResolver.parseSectionID` を抽出（id wire-format の単一所有者）。`Backend.setSectionLens(_ id:)` rename。`catalog.activeSectionLens` が id 格納。adapter に `lensSection(forID:)`/`activeLensSection()` 新設・4 つの label scan（`lensLayout`/`resolvedLensLayout`/`activeSectionLensApplyForward`/`sectionLensFilter`）を id 解決へ。Controller に `lensID(forLabel:ordinal:)`（config-based declOrder）+ `setActiveLens`→`activateLensID(id-core)` 分離・`toggleActiveLens`/`setLensLayout`/`addressableSections`/Grid・Rail onPick を id 経由へ。
  - **reorder hot-reload drop = 許容 + `Log.line`**（トミー 2026-06-25 確定）: config の lens 並べ替え→reload で declOrder 変化→id 解決不能→workspace へ drop（今 label scan で残る分の差・狭く良性）。`reloadConfig` が fresh config へ `ApplyResolver.section(forSectionID:)` で再検証し drop 時に `Log.line`（今 silent な label 削除 drop も可視化）。**drop は backend lens も clear して park 窓を restore＝完結**（実機検証で発覚: mirror だけ落とすと catalog の lens が残り窓が park されたまま Controller↔backend desync する pre-existing gap。`runBackendCommand{ setSectionLens(nil) }` を `updateConfig` 前に enqueue＝FIFO で旧 config 下に clear→restore。`setActiveLens(nil)` の clear と同型）。
  - **検証**: `swift build` green・`swift test`（Xcode 26.5）全緑（lens-touched 98 tests 含む・SIGSEGV は既知 flaky の一過性で再現せず）・adapter の lens label scan grep 0 件。**実機自動検証 3/3 PASS**（Test A: `facet lens Web`→`section:3:Web` visible=2 parked=1 + tree highlight / Test B: `section --focus 5`→`section:4:Code`・`--focus Web`→`section:3:Web` / Test C: reorder→`active lens no longer resolves … → workspace 1` Log.line + drop）。reorder-drop 完結 fix は build+全緑+threading 解析で検証（live 再確認は active Space が 15 から drift＝5k2k flapping で未・desktop 15 active 時に最終目視可）。

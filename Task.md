...
- ✅ **A0 lens identity を安定 id へ decouple** 完了（挙動不変リファクタ・`feat/a0-lens-identity-id`）。lens の同定キーを label 文字列 → **安定 id `"section:<declOrder>:<label>"`**（= `ProjectedSection.id`・declOrder = config FULL 配列の index）へ移行。**スコープ = minimal**（識別は Core/Adapter/Controller のみ id 化・**view モジュールは label 据え置き**＝pick `.lens(label:)`・highlight `sec.label==activeLens` のまま、`lastActiveLens=currentActiveSection.lensLabel` で id から label を parse して供給。view の id 化は A で実施）。
  ...
  - **reorder hot-reload drop = 許容 + `Log.line`**（トミー 2026-06-25 確定）: config の lens 並べ替え→reload で declOrder 変化→id 解決不能→workspace へ drop（今 label scan で残る分の差・狭く良性）。`reloadConfig` が fresh config へ `ApplyResolver.section(forSectionID:)` で再検証し drop 時に `Log.line`（今 silent な label 削除 drop も可視化）。
  - **検証**: `swift build` green・`swift test`（Xcode 26.5）全緑（lens-touched 98 tests 含む・SIGSEGV は既知 flaky の一過性で再現せず）・adapter の lens label scan grep 0 件。実機目視（`facet lens NAME`・tree toggle・`section --focus index:N|label:NAME`・reorder reload drop+log）はトミー側 wss≥1 時。

...
- [~] **R12. mac desktop 切替（ctrl+→/←）後、tree のキーボード操作が効かない**（トミー実機報告 2026-06-24）— **🔧 段階1（thrash 修正）+ 段階2（click-to-activate）実装・branch `fix/r12-tree-kbnav-after-desktop-switch`／実機検証中**。
  ...
  - **📋 実機検証（トミー）**: ① どの切替後でも tree を**クリック→キーボード操作開始**するか。② **13→14（未管理 desktop 経由）でクリック→復帰**するか（本報告の核心）。③ active な tree の窓行クリックは従来どおり**窓フォーカス**するか（#66 退行なし）。ログに `panelKey gained (kbNav=false)` 直後 `enterKbNav` 相当が出れば成功。

...
### rail view
全画面の **[[workspace]] overview**（Mission Control 風）。`--view=rail` で召喚。
...
### sticky window
1 つの window を **現在の native Space 内・全 facet workspace のメンバー**
...
native Space 跨ぎは対象外（READ-only SkyLight・macOS の「すべてのデスク
トップ」任せ）。session 限り・per-native-Space・`marks` と直交。
...
- spawn なし（既存窓の出し入れのみ・launcher 化しない＝rules engine 領域は
  ...
  `forgetWindow` が自動 prune / session 限り・per-native-Space。

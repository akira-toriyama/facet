...
（"タグ集合" × "いま active な唯一の section"）として綴り分ける。
...
| **lens** | 二義語。① tag モードで「今見えているタグ集合」（bitmask・M11-3・#176）／② section モデルの cross-workspace exclusive な可視フィルタ（`type=lens` section）。`facet lens` 動詞を共有するが grouping が排他なので両者が同時に active になることはない | `facet lens`, `WorkspaceCatalog.lensOnly` / `lensToggled`（①）, `WorkspaceCatalog.activeSectionLens`（②） |
...
[[active lens]]（`by="workspace"`・positional `facet lens NAME`）とは別概念（grouping が
排他なので同時に有効化しない・`facet lens` subject を共有するだけ）。

**section モデルの lens（②）**: `by="workspace"` 下で `type=lens` section を
`facet lens NAME` で active 化すると、その `match` を満たさない窓を **全 workspace
横断で [[anchor]] park**（cross-workspace exclusive な実 hide）し、一致する窓集合を
その lens の `layout`（stateless engine）で **union タイル**する。これは [[active lens]]
＝唯一の [[active section]] を選ぶ操作であり、workspace を切り替えると lens は
自動解除される。詳細は [[active section]] / [[active lens]] 参照。
- CLI: `facet lens NAME`（= 旧 `--only`・positional・CSV `A[,B,…]`）/
  `--add`/`--remove`/`--toggle A[,B,…]` / `--all` / `--clear`。tag-unification
  Phase 1 で `--only`/`--section` を **positional NAME に統一**（両モード共通動詞）。
  tag モードの `--clear` は floor へ戻す＝`--all` と同じ全窓表示（section モデルの
  「active lens 解除」と統一文言）。`--add`/`--remove`/`--toggle`/`--all` は tag 専用。
...
- **3 つの「add a tag」を混同しない**（用語規則）：`facet tag --add` = **語彙**にタグを定義（窓は触らない）／
  `facet window --tag` = **窓**にタグを付ける（未定義なら auto-vivify）／`facet lens NAME`（旧 --only）/ `--add`/`--remove`/`--toggle` = **表示集合**を変える
...
（keyboard nav で `t`・[[grouping]] `by=tag` 時のみ）。`s`（検索モード）の双子＝窓に
...
ゲートなし）。**`facet lens NAME`（旧 --only）/ `--add`/`--remove`/`--toggle`/`--all`/`--clear`
がタグ表示集合を変える write verb なのに対し、`query --lens` はその集合を読むだけ**
（read ↔ write の別物）。
...
- **grid/rail**: park された窓のサムネを **`Window.isLensParked` で直接 drop**（EX-0 で
  横断化＝**全 workspace** の park 窓が drop される・catalog が単一権威で view 側の
  `match` 再計算なし）。セル数は **不変**（lens はセル内の窓を絞るだけ・再束ね/セル
  削除はしない）。⚠️ grid/rail の lens セル化＋ハイライト統一（lens active 時に
  active workspace セルを消灯）は **EX-2** 予定（現状は active workspace セルが点灯のまま）。
...
- CLI: `facet lens LABEL`（activate・positional NAME）/ `facet lens --clear`（解除）。
  ...
  `lastError`）・`by="workspace"` 必須（tag verb と排他）。`facet lens NAME` は両モード
  共通動詞で、tag モードでは tag 表示集合を選ぶ（[[lens]] 参照）。

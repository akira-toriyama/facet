...
plus **[[facet workspace]]**（各 section の空間セル）と **[[lens desktop]]**
（`[desktop.N] type=lens` の常時フィルタ desktop）。**lens は今や desktop の型で
あって section ではない**（旧 section-lens は t-ec9s で退役）。
...
| 正規名 | 意味 | コード / 設定での所在 |
|---|---|---|
| **mac desktop** | macOS の native Space（OS の仮想デスクトップ。Mission Control の "Desktop N"）。**型付き**（`workspace` / `lens`） | `MacDesktops`, `activeMacDesktopID`, `[desktop.N]` / `[[desktop.N.section]]` |
...
| **lens desktop** | `[desktop.N] type=lens` の **常時有効な filtered mac desktop**。`match` の窓を `layout` でタイルし残りを park（tree 専用）。**lens はこの desktop 型としてのみ存在** | `desktopType`, `desktopLens`, `applyIsolatePark` |
...
区別し続ける。**facet workspace ↔ [[lens desktop]]** も別物（前者は窓を並べる
...
- 設定: `[[desktop.N.section]]` / `[desktop.N]`（typed desktop・[[lens desktop]]）
...
### lens desktop
`[desktop.N] type = "lens"` で型付けした [[mac desktop]]＝**常時有効（always-on）の
filtered desktop**。階層は **mac desktop > section > window**（旧 **board** 層は
t-0sbm で廃止——「workspace と lens を両方使いたい」は board でなく **mac desktop を
...
- **挙動**: その mac desktop に居る間ずっと、`match` に合う窓を `layout` でタイルし、
  ...
  （`effectiveWorkspaceList` は N=1 の 1 slot だけを seed）。park は**実画面だけ**の
  操作（tree に何を並べるかは `show-non-matching` が決める——`true` で park された
  非マッチ窓も "holding" section に載り全窓 inventory になる）。
- **tree 専用**（2世界分離）: lens は動的（membership が変わる）で固定画面が無い→
  サムネイル不能。`--view grid` / `--view rail` はその desktop 上では **loud reject**。
  `show-non-matching = true` で tree が matched + "holding"（非マッチ）の 2 section
  表示（既定 false = matched の 1 section）。
- 設定: `[desktop.N]`（SINGLE table・`[[…]]` 配列ではない）: `type` 必須／`lens` は
  `match` 必須・`layout`・`show-non-matching`・`label` 任意
- コード: `DesktopMeta` / `FacetConfig.desktopType`・`desktopLens`・
  `lensDesktopSections`（1|2 section 合成・純）/ `NativeAdapter.applyIsolatePark`
  （always-on park + lens-layout seam）/ `WorkspaceCatalog.isolateParked` /
  `IsolatePark.parkSet`（純）
- **Don't call it:** board（旧概念・t-0sbm で廃止）, tab, focus mode, lens board,
  filtered space, フォーカスモード
...
（どう見せるか）は [[facet workspace]] / [[tag]] / [[lens desktop]]（何を見せるか）
とは直交する別軸。
- CLI: `--view NAME` / `--hide NAME` / `--toggle NAME`
- **Don't call it:** mode, panel, window, lens（lens は今や [[lens desktop]]・view とは別概念）,
  モード, ペイン

### lens
**保存された window フィルタ**の総称。facet では **今や [[lens desktop]] としてのみ
存在する**＝`[desktop.N] type = "lens"` で型付けした常時有効な filtered mac desktop
（`match`〔[[facet filter]] WHERE 式・例 `tag~=web or app=Safari`〕の窓を `layout` で
タイルし、残りを anchor-park・tree 専用）。かつて存在した "`type="lens"` section＝
`facet lens NAME` で active 化する pure VIEW"（`activeSectionLens` / `--clear`）は
**退役**した（t-ec9s）。窓を絞り込む述語は [[facet filter]]、それをタグ付けに使う宣言的
ルールは [[rule]]、window のタグ自体は [[tag]] が担う。lens desktop の `match` を runtime で
retarget するのは `facet section --match N "expr"`（session-only）／GUI は tree ヘッダの
Section ▸ Edit match。
- 設定 / コード: [[lens desktop]] 参照（`[desktop.N] type=lens`・`desktopLens` /
  `applyIsolatePark`）
- **Don't call it:** view（facet view は UI surface の別概念）, section-lens / `type="lens"`
  section（退役）, tagset, タグビュー, saved filter
...
NAME を持つ窓を集める（[[lens desktop]] / [[rule]] の `match`・`facet query --filter`）。
...
[[lens desktop]] の `match`・[[rule]] の `match`・`facet section --match` が **1 つの文法**を
...
- コード: `FacetFilter`（AST + `parse` + `matches` + `description`）/ `WindowFields`（窓 → field
  ...
  `facet query --filter`。[[lens desktop]] の `match` もこの言語を共有する。
...
[[lens desktop]] ではその always-on な合成 section。`ActiveSection`
...
かつての `type = "lens"` section（保存可視性フィルタ）は **退役**した（t-ec9s）＝lens は今や
[[lens desktop]] としてのみ存在する。
...
- **Don't call it:** group（旧称＝旧型名 `FilterGroup`）, lens / `type="lens"` section（退役・
  lens は今や [[lens desktop]]）, tab, page, グループ, セクション以外
...
[[unassigned]] の先頭窓 focus。[[lens desktop]] 上では合成 section の先頭窓 focus）、
...
label〕・relaunch で reset・`facet reload` では消えない。lens desktop 上では reject）、
`--match N "expr"` で [[lens desktop]] の `match` を runtime retarget（session-only・
...
### match
[[lens desktop]] / [[rule]] が共有する **述語キー**＝当たった窓を lens desktop がタイル
...
- コード: lens desktop の `match`（`desktopLens`）/ `Rule.match`（生文字列）→

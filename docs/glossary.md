...
| **lens** | workspace と直交する **保存された可視フィルタ＝SQL VIEW**（`type="lens"` section）。`facet lens NAME` で active 化すると tree/grid/rail の **表示だけ**が変わり、現 mac desktop の全 workspace 横断で match した窓を集約表示する（実 OS 窓は動かさない） | `facet lens`, `WorkspaceCatalog.activeSectionLens` |
...
**迷子は稀**＝t-qtpx が ws→lens DnD の「全部移動（迷子化）」を撤去したので、迷子を
...
### lens
workspace と直交する **保存された可視フィルタ＝SQL VIEW**＝`type="lens"` の
[[section]]。`facet view` が "どう見せるか" なのに対し
lens は "どの窓集合を見るか"＝直交する別軸。`facet lens NAME` で active 化すると、
tree/grid/rail の **表示だけ**が変わる：`FilterProjection` がその `match`（[[facet filter]]
WHERE 式・例 `tag~=web or app=Safari`）を満たす窓を、現 mac desktop の **全 workspace 横断で
集約表示**する。**実 OS 窓は一切動かさない**（park もタイルも起こらず・実画面は不変）。
...
hero が追従する。**EX-2b: 中央が lens section なら hero はその一致窓を集約表示する**
（cross-workspace の match 結果をまとめて見せる・[[active section]] = 中央 hero の
不変則を維持・トミー 2026-06-22）。
...
- **type=workspace**: 常設の空間土台（タイル単位・grid/rail のセル）。**任意の
  ...
  `layout` seed も持つ。**user 記述の `match` / `apply` は禁止**（t-qtpx・
  warn+drop。所属は DnD / `facet window --move-to N` で変える）。
...
`unassigned = true` 受け皿では任意（既定 workspace・投影に無関係）。section 未定義の
...
`removeTag` は decode されない）。**全 op を持てるのは `[[rule]]` のみ**＝
`type="lens"` section の `apply` は **tag 付与のみ**（t-qtpx・`workspace`/`floating`/
`sticky`/`master` は warn+drop）、`type="workspace"`/`unassigned` は `apply` 自体禁止。**PR8 で結線済**＝`ApplyResolver`（`FacetCore`・pure）が
...
### active lens
section/lens モデルで **いま選択中の [[section]]（type=lens）**（pivot・t-0021 で
**PURE VIEW** 化）。**単一**（同時 active な lens は 1 つ）・
...
- **表示フィルタ（核）**: active lens の `match` を満たす窓を、現 mac desktop の
  **全 workspace 横断で集約表示**する（`FilterProjection` が tree/grid/rail の表示を
  差し替えるだけ）。**実 OS 窓は一切動かさない**（park もタイルも起こらず・実画面は
  不変）。一致窓へ作用するにはそれを **クリック**すれば通常の窓 focus 経路で jump する
  （workspace 切替＋focus）。**catalog が権威**（`activeSectionLens`）・`--clear`（または
  別 workspace への切替）で view を捨てる。`layout` キーは workspace section でのみ意味を
  持ち、lens では parse されるが **無視**（lens は何もタイルしない・layout picker 無し）。
- **[[tree view]]**: active lens の section ヘッダを **単独で**強調（`pal.primary` +
  ...
  **その highlight ミラー**（catalog の権威を読み戻すだけ）。一致窓行を **クリックすると
  その窓へ jump**（通常の窓 focus 経路＝workspace 切替＋focus）。
- **grid/rail**: tree と同じ順序の section リストを描画する＝**lens section も独立セル**
  ...
  `activateSection` 経由（workspace 切替 or lens 起動＝表示切替・`RailPick`/`GridPick`）。
  **rail は active／選択 section を中央 [[hero]] に**＝lens が中央なら hero がその一致窓を
  集約表示する（トミー 2026-06-22）。lens セルは browse 可だが drag/swap 対象外（grip なし・
  layout picker なし）。これで **3-view 統一ハイライト**（tree=EX-1／grid=EX-2a／rail=EX-2b）
  が完成。
...
- **lens を active 化**（`facet lens NAME`・lens セル/窓のクリック）すると、その
  workspace highlight を消し、`match` 一致の窓を **全 workspace 横断で集約表示**する
  （**表示だけ**・実 OS 窓は動かさない）。

...
struct RealWindowResizeTests {
    ...
    @Test func resizeRightEdgeGrowsAndNeighborFollows() {
        ...
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 1000, height: 900))
        #expect(f[wid(2)] == CGRect(x: 1000, y: 0, width: 600, height: 900))
    }

    @Test func resizeRightEdgeShrinks() {
        var t = twoVertical()
        t.resize(wid(1), to: CGRect(x: 0, y: 0, width: 400, height: 900),
                 in: rect)
        let f = t.frames(in: rect)
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 400, height: 900))
        #expect(f[wid(2)] == CGRect(x: 400, y: 0, width: 1200, height: 900))
    }

    @Test func resizeNeighborLeftEdgeSameDivider() {
        // Dragging window 2's LEFT edge moves the same fence.
        var t = twoVertical()
        t.resize(wid(2), to: CGRect(x: 600, y: 0, width: 1000, height: 900),
                 in: rect)
        let f = t.frames(in: rect)
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 600, height: 900))
        #expect(f[wid(2)] == CGRect(x: 600, y: 0, width: 1000, height: 900))
    }

    @Test func resizeBottomEdgeHorizontalSplit() {
        var t = twoHorizontal()                  // 1=(0,0,900,800)
        t.resize(wid(1), to: CGRect(x: 0, y: 0, width: 900, height: 1000),
                 in: tallRect)
        let f = t.frames(in: tallRect)
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 900, height: 1000))
        #expect(f[wid(2)] == CGRect(x: 0, y: 1000, width: 900, height: 600))
    }

    @Test func resizeCornerUpdatesBothFences() {
        ...
        t.resize(wid(1), to: CGRect(x: 0, y: 0, width: 1000, height: 1000),
                 in: r)
        let f = t.frames(in: r)
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 1000, height: 1000))
        #expect(f[wid(2)] == CGRect(x: 1000, y: 0, width: 600, height: 1000))
        #expect(f[wid(3)] == CGRect(x: 0, y: 1000, width: 1600, height: 600))
    }

    @Test func resizeScreenEdgeIsNoOp() {
    ...
    @Test func resizeClampsRatio() {
        ...
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 1520, height: 900))
        #expect(f[wid(2)] == CGRect(x: 1520, y: 0, width: 80, height: 900))
    }

    @Test func resizeAbsentIsNoOp() {
        let t0 = twoVertical()
        var t = t0
    ...
    @Test func resizeReturnsFreezeSetSimpleFence() {
    ...
    @Test func resizeHighFenceFreezesWholeSubtree() {
    ...
    @Test func catalogApplyResizeBsp() {
        ...
        #expect(c.applyResize(
            wid(1), to: CGRect(x: 0, y: 0, width: 1000, height: 900),
            workspace: 1, in: rect) != nil)
        let f = c.tiledFrames(for: 1, in: rect)
        #expect(f[wid(1)] == CGRect(x: 0, y: 0, width: 1000, height: 900))
        #expect(f[wid(2)] == CGRect(x: 1000, y: 0, width: 600, height: 900))
    }

    @Test func catalogApplyResizeTallMaster() {
        var c = seeded(3)
    ...
    @Test func catalogApplyResizeNoOpGrid() {
        var c = seeded(2)
    ...
    @Test func catalogApplyResizeNoOpUnmovedEdge() {
        var c = seeded(3)
    ...
    @Test func catalogApplyResizeGapNoSpuriousCrossAxis() {
        ...
        #expect(c.applyResize(
            ...
            workspace: 1, in: rect, innerGap: 20) == nil)
        let f = c.tiledFrames(for: 1, in: rect)
        #expect(f[wid(1)]?.maxX == 800)   // divider untouched
        #expect(f[wid(2)]?.minX == 800)
    }

    @Test func catalogApplyResizeBspWithGap() {
        ...
        #expect(c.applyResize(
            ...
            workspace: 1, in: rect, innerGap: 20) != nil)
        let f = c.tiledFrames(for: 1, in: rect)
        #expect(f[wid(1)]?.maxX == 1000)
        #expect(f[wid(2)]?.minX == 1000)
    }
}

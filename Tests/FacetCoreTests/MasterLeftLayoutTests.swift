...
final class MasterLeftLayoutTests: XCTestCase {
    ...
    func testEmptyOrderEmptyFrames() {
        XCTAssertTrue(ml.frames(order: [], focused: nil,
                                params: LayoutParams(),
                                in: screen).isEmpty)
    }
    ...
    func testRegistryResolvesMasterLeft() {
        XCTAssertEqual(LayoutRegistry.engine(named: "master-left")?.name,
                       "master-left")
        XCTAssertEqual(LayoutRegistry.engine(named: "MASTER-LEFT")?.name,
                       "master-left")
        XCTAssertTrue(LayoutRegistry.names.contains("master-left"))
    }
}

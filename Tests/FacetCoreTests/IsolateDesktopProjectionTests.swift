...
/// `FilterProjection.projectIsolateDesktop` (board abolition t-0sbm → section-lens
/// removal t-ec9s) — a `type="lens"` mac desktop projects DIRECTLY, without
/// synthesizing a config `DesktopSection`: ONE `.matched` section (matched windows,
...
struct IsolateDesktopProjectionTests {
    ...
    @Test func showFalseYieldsOnlyTheMatchedSection() {
        ...
        #expect(r.sections[0].sectionType == .matched)
    ...
    @Test func showTrueHoldsNonMatchingInReceptacle() {
        ...
        #expect(r.sections[1].sectionType == .holding)

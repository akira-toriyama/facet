...
/// TallLayout in `horizontal` orientation = Wide (master row on top,
/// stack columns below). 1600×1000, ratio 0.5 → master row 500 tall,
/// columns 800 wide.
final class WideLayoutTests: XCTestCase {
    ...
    private func wide(_ ratio: CGFloat = 0.5, masters: Int = 1) -> LayoutParams {
    ...
    func testMultipleMastersAsTopColumns() {
        let f = tall.frames(order: [wid(1), wid(2), wid(3), wid(4)],
                            focused: nil, params: wide(masters: 2),
                            in: screen)
    ...
    func testRatioControlsMasterHeight() {
        let f = tall.frames(order: [wid(1), wid(2)], focused: nil,
                            params: wide(0.6), in: screen)
        XCTAssertEqual(f[wid(1)]?.height, 600)   // 0.6 * 1000
        XCTAssertEqual(f[wid(2)]?.minY, 600)
        XCTAssertEqual(f[wid(2)]?.height, 400)
    }

    func testVerticalAndHorizontalDiffer() {
        let order = [wid(1), wid(2)]
        let v = tall.frames(order: order, focused: nil,
                            params: LayoutParams(), in: screen)
        let h = tall.frames(order: order, focused: nil,
                            params: wide(), in: screen)
        XCTAssertNotEqual(v[wid(1)], h[wid(1)],

...
final class MasterRightLayoutTests: XCTestCase {
    ...
    func testTwoWindowsMasterOnRight() {
        let f = mr.frames(order: [wid(1), wid(2)], focused: nil,
                          params: LayoutParams(masterRatio: 0.5),
                          in: screen)
        XCTAssertEqual(f[wid(1)], CGRect(x: 800, y: 0, width: 800, height: 1000))
        XCTAssertEqual(f[wid(2)], CGRect(x: 0, y: 0, width: 800, height: 1000))
    }

    func testThreeWindowsStackRowsOnLeft() {
    ...
    func testIsXMirrorOfMasterLeft() {
        for n in [2, 3, 4, 5] {
            ...
            for id in order {
                XCTAssertEqual(right[id], left[id].map { mirrorX($0, in: screen) },
                               "master-right must be master-left mirrored across X (n=\(n))")
            }
        }
    }

    func testRegistryResolvesMasterRight() {
        XCTAssertEqual(LayoutRegistry.engine(named: "master-right")?.name,
                       "master-right")
        XCTAssertTrue(LayoutRegistry.names.contains("master-right"))
    }
}

...
public func resolveTreeDrop(
    ...
    switch ctx.sourceID {
    case .header(let sid):
        guard sectionMode, !isolateDesktop,
        ...
        guard secs.firstIndex(where: { $0.id == sid }) != nil else { return nil }
    ...
    case .window(let g, let wid):
        ...
        // Destination section ordinal: `.onto` a row → that row's section;
        // `.between` → the section the gap belongs to (the gap ABOVE a header
        // closes the PREVIOUS section; the end gap is the last section).
        let dest: Int? = {

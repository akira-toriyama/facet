...
/// Convert a raw scroll-wheel `deltaY` into an integer step count for a
/// section switcher — the shared "precise vs notched" accumulation math that
/// drives the `RailView` carousel (t-9amp / R1; the retired board bands were
/// the other consumers). Pure / total; lives here so it's unit-testable.
///
...
///   apply hands the net straight to its index-step helper.

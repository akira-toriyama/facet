...
extension NativeAdapter {
    ...
    ///
    /// EX-0.2 fix: the section-lens and tag-mode union branches mirror
    ...
    /// violates トミー's 負債を残さない principle. Both branches ignore `n1Based`
        ...
        -> [WindowID: CGRect]
    {
        ...
        if catalog.grouping == .tag {
            return catalog.tagUnionFrames(in: rect)
        }
